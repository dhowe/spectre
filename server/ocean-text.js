...
let conscientiousness = {
  ...
  text: [
    ...
    '$user.name.ucf() $user.toBe() a perfectionist. $user.pronoun().ucf() prefers to plan everything to the last detail, which has consequently led to $user.pronoun() being very successful and extremely reliable. $user.pronoun().ucf() enjoys seeing $user.poss() long-term plans come to fruition.'
  ]
}
...
let extraversion = {
  ...
  text: [
    '$user.name.ucf() $user.toBe() quiet and somewhat withdrawn. $user.pronoun().ucf() is someone who doesn’t need lots of other people around to have fun, and can sometimes find that people are tiring.',
...
let agreeableness = {
  ...
  text: [
    '$user.name.ucf() $user.toBe() willing to make difficult decisions when necessary, and will point out when something $user.toBe() wrong no matter what other people might feel. (One|You) would say that $user.pronoun() can be tough and uncompromising.',
    '$user.name.ucf() people can find difficult to get along with when $user.pronoun() first meet, as $user.pronoun() can be suspicious of their motives. Over time though people warm to $user.pronoun(), and $user.pronoun() to them, although that doesn’t stop $user.pronoun() telling them "how it $user.toBe()".',
    ...
    '$user.name.ucf() is someone people get along with easily. $user.pronoun() $user.toBe() considerate and friendly, and expect others to be generally honest and decent.',
    '$user.name.ucf() $user.toBe() extremely easy to get along with. $user.pronoun() $user.toBe() considerate, friendly, generous and helpful and $user.pronoun() considers most others to be decent and trustworthy.'
  ]
}
...
let neuroticism = {
  ...
  text: [
    '$user.name.ucf() $user.toBe() extremely difficult to upset or stress out, since $user.pronoun() rarely, if ever, react with negative emotions, and even when $user.pronoun() $user.toBe() anxious about something the feeling quickly passes. $user.pronoun() comes across as very calm and resilient.',
    '$user.name.ucf() $user.toBe() calm and emotionally stable. $user.pronoun().udc() comes across as someone who $user.toBe() rarely bothered by things, and when they do get $user.pronoun() down the feeling does not persist for very long.',
    ...
    '$user.name.ucf() tends to be more self-conscious than many. $user.pronoun().ucf() comes across as someone who can find it hard to not get caught up by anxious or stressful situations. (One|You) might say that $user.pronoun() $user.toBe() in touch with $user.poss() own feelings.',

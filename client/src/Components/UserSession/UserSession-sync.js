...
UserSession.disableLocalStorage = true;
  ...
  if (props.includes('_id') && typeof user._id === 'undefined') {
    ...
    if (!sid) {
      ...
      UserSession.create(user, (u) => {
        if ((props.includes('target') || props.includes('similars')) && !u.similars.length) {
...
UserSession.fillMissingProperties = function(user, props) {
  ...
  const checkTargetAdData = (missing) => {
    if (!user.target._id || !user.adIssue) return;
    if (!(user.targetImages && user.targetImages.length &&
      ...
      user.targetInfluences && user.targetInfluences.length)) {
        ...
        arrayRemove(missing,
          'adIssue', 'targetImages',  'targetSlogans', 'targetInfluences');
      }
  }
...
UserSession.create = function(user, onSuccess, onError) {
  ...
  let internalSuccess = (json) => {
    Object.assign(user, json);
    if (!UserSession.disableLocalStorage) sessionStorage.setItem
...
function arrayRemove(a, v) { return Array.isArray(v) ?
